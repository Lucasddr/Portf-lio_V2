...
export default function Certificates({certificates} : CertificatesProps) {
    return (
        <div className="bg-(--surface-light) rounded-2xl px-3 py-4 border border-(--muted)/20">
            <div className="flex flex-col gap-4">
                <div className="w-26 px-2">
                    <img className="rounded-2xl" src={certificates.img}/>
                </div>
                <div className="px-2">
                    ...
                    <a className="text-(--primary) text-(length:--font-md) flex gap-1" href={certificates.link}>Ver Certificado <ArrowUpRight className="-translate-y-0.5"/></a>
                </div>
            </div>
        </div>
    )
